...
def convert(args, D, directory):
    ...
    # print()
    for in_fn in sorted(glob(os.path.join(args.GLUE, directory) + '/*.csv')):
        # print(in_fn)
        ...
        with open(in_fn, 'r', encoding='utf-8-sig') as infile:
            ...
            read_tsv = source_df.iterrows()
            # for row in tqdm(read_tsv, total=total_length):
            failed_counter = 0
            for row in read_tsv:
                # print(len(row))
                # row = re.match(r"([a-z]+)([0-9]+)", row, re.I)
                # print('redone row: ', row)

                if not wrote_header:
                    header = row
               ###### write_header(row, D.modification_counter.keys(), outfile)
                    col_list =  source_df.columns.values.tolist()
                    for mod in D.modification_counter.keys():
                        col_list += ['sentence_' + mod]
                    ...
                    # print(row)
                    # print(len(row))
                    # [row] = re.match(r"([0-9]+)([a-z]+)", row[0], re.I)
                    # print('redone row: ', row)
                    # if len(header)!=len(row):
                    #     print('failed', row)
                    #     print('row len', len(row))
                    #     print('len header not equal len row')
                    #     print('header:', header)
                    ...
                    for i, key in enumerate(col_list):
                        ...
                        if ('sentence_' in key):
                            ...
                            # print('ROW: ', row)
                            # print(row)
                            revised = D.convert_sae_to_dialect(row[1]['sentence'])
                            # print(revised)
                        ...
                        #     out.append(str(row[i]))
                            ...
                            # failed_counter +=1
                    # print(r)
                    out_df.loc[len(out_df.index)] = r

                    if failed_counter >= 3:
                        exit()
                    out_df.to_csv('/u/camilleh/value/data/VALUE/' + basename)
